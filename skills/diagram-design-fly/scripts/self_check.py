...
# Continuous modes declare no steps and ship no controller. `flow` is the mode
# for movement with no beginning or end; see references/flow.md.
...
def check_motion(parser: DiagramParser, source: str, errors: list[str]) -> None:
    ...
    if mode == "flow":
        # Continuous motion has no ordered sequence to number, so a step count
        # would imply a controller and a narrative that do not exist.
        count = 0
        if raw_count:
            errors.append(
