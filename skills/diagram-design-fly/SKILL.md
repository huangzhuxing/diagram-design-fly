...
- Optional explanatory motion, ordered → [animation.md](references/animation.md)
- Optional continuous flow, pipelines and cycles → [flow.md](references/flow.md)
...
| Max flow (optional) | 8 connectors, 16 tokens, 2.5–6s cycle — see [flow.md](references/flow.md) |
...
**Flow** (optional, for continuous movement) — see [flow.md](references/flow.md). Start from `assets/template-flow.html`. Mode `flow` is CSS-only: no script, no controls, and it shares the static budget rather than raising it.

### To create a new diagram

1. Copy the variant closest to what you want (`assets/template.html` for minimal, `assets/template-full.html` for cards, `assets/template-motion.html` for stepped motion, `assets/template-flow.html` for continuous flow).
...
4. If the subject carries an order, load `flow.md` and build it in `flow` mode with `data-flow-sequence` on — that is the default here, not an upgrade the user has to ask for. Load `animation.md` only for a stepped explanation that finishes. Otherwise keep mode `none`.
...
### Exporting to MP4 / WebM / GIF

A `flow` diagram also renders to video — load [`references/video-export.md`](references/video-export.md). **Manual only**, never unprompted; a static PNG stays the default. Reach for video only when the movement is the content. `step` and `reveal` are JavaScript-driven and render as a still, so export those as PNG.
